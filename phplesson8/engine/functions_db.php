...
function authors_get_id_and_rights($link, $log, $pas){
		$log = htmlspecialchars(strip_tags(trim($log)));
		$pas = trim($pas); 

		$query = sprintf("SELECT id_author, author_pass,rights FROM authors WHERE author_login='%s'",$log);
		//echo $query . '<br>';
		$result = mysqli_query($link, $query);
